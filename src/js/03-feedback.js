...
// код
lastFormInput();
 // функція відновлює останні дані, що було до перезавантаження сторінки

// сет функцій
function onLocalStorageSet(evt) {
  let savedData = localStorage.getItem('feedback-form-state');
  savedData = savedData ? JSON.parse(savedData) : {}; 
  //перевіряємо якщо дані в LocalStorafe є, то перезаписуємо їх, якщо немає, створюємо обєкт з такими властивостями
  savedData[evt.target.name] = evt.target.value; 
  //зберігаємо введені дані як властивість у об'єкт
